...
- 负责分流。
    - 直连：待解析域名在 [direct.txt](https://raw.githubusercontent.com/217heidai/RoutingRules/main/rules/direct.txt)、direct_custom.txt(自定义直连域名)内，通过 SmartDNS 的 mainland 组进行解析。
    - 代理：待解析域名在 [proxy.txt](https://raw.githubusercontent.com/217heidai/RoutingRules/main/rules/proxy.txt )、proxy_custom.txt(自定义代理域名)内，通过 SmartDNS 的 gfw 组进行解析。
    - Fallback：待解析域名不在以上规则内的，先使用 SmartDNS 的 mainland 组进行解析，如解析出来的地址为国内 IP(在 [ipv4_china.txt](https://raw.githubusercontent.com/217heidai/RoutingRules/main/rules/ipv4_china.txt) 内)则使用，否则抛弃。再使用 SmartDNS 的 gfw 组进行解析，如解析成功则使用，如失败则使用 SmartDNS 的 overseas 组进行解析（gfw 组使用代理连接公共 DNS，当代理异常时使用 overseas 组作为备用进行解析）。
...
- 自定义直连域名文件为 `$HOME/nestingdns/etc/site/direct_custom.txt`。默认为空，可自行添加内容，如填入 VPS 的域名。
- 自定义代理域名文件为 `$HOME/nestingdns/etc/site/proxy_custom.txt`。默认为空，可自行添加内容。
...
- gfw TTL 默认全部调整为最大 300s，`$HOME/nestingdns/etc/conf/mosdns_forward.yaml` 中设置 sequence dns_gfw，***如有多个代理地址切换使用的，请设置较小值，如 60***。（上游 smartdns gfw 组默认已关闭缓存、测速）
